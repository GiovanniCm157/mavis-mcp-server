...
**NINGUNA** en B-6. La doctrina actualizada: **MiniMax hace TODO, vos solo pensás**. Aunque `mavis_auditor`, `mavis_noter` y `mavis_session_log` no consuman tokens de MiniMax-M3, las invocás a través de `mavis_coder_agent` igual. El LLM decide cuándo usarlas.

Para TODO (incluidas `mavis_auditor`, `mavis_noter`, `mavis_session_log`): **SIEMPRE via `mavis_coder_agent`**.
...
**Recursion guard** (B-6): por default, **SOLO** `mavis_coder_agent` está excluido (auto-recursión = loop infinito). `mavis_coder` (single-shot) y TODAS las otras 13 tools (incluidas las non-LLM: `mavis_auditor`, `mavis_noter`, `mavis_session_log`) están expuestas por default. Para override, pasalas explícitamente en `tools`.
